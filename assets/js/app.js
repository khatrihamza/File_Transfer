...
window.onPeers = (data) => {
  ...
  for (let it of items) {
    ...
    li.innerHTML = `<div><strong>${name||'Unknown'}</strong><div class="muted">${addr||''}</div></div><div><button class="btn" onclick="pasteAddr('${addr}')">Use</button></div>`;
...
window.onProgress = (p) => { progressBar.style.width = Math.min(100,p)+'%'; };
window.onIncoming = (path) => { showModal('Received','Saved to: '+path); };
window.onTransferDone = (s) => { showToast('Transfer '+s); progressBar.style.width='0%'; };
